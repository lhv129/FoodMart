...
<div class="container-fluid">
    ...
    <!-- Page Heading -->
    <h1 class="h3 mb-2 text-gray-800">Chỉnh sửa đơn nhập hàng</h1>
    <!-- DataTales Example -->
    <div class="card shadow mb-4">
        <div class="card-body">
            <h5 class="mt-2">Chi tiết đơn hàng</h5>
            <form class="col-xl-4 col-lg-5 col-md-6 col-12" method="POST" enctype="multipart/form-data" action="{{ route('admin.good.details.edit.store', $goodReceiptNote->id ) }}">
                @csrf
                            ...
                            <table class="table table-bordered dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%;">
                                ...
                                <tfoot class="text-center">
                                    <tr>
                                        ...
                                        <th rowspan="1" colspan="1">{{ number_format($goodReceiptNote->total_price, 0, ',', '.') }} vnđ</th>
                                        <th rowspan="1" colspan="1"></th>
                                    </tr>
                                </tfoot>
                                <tbody class="text-center">
                                    ...
                                    <tr class="odd">
                                        ...
                                        <td @if ($loop->last) class="" @else class="text-center" @endif>
                                            <form method="POST" action="{{ route('admin.good.details.edit', $item->id) }}">
                                                @csrf
                                                @method('DELETE')
        ...
        <div class="card-body">
            <h4>Tạo phiếu</h4>
            <div class="row">
                <form class="col-xl-4 col-lg-5 col-md-6 col-12" method="POST" enctype="multipart/form-data" action="{{ route('admin.goods.update', $goodReceiptNote->code) }}">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="supplier_id">
                            ...
                            <option value="{{ $supplier->id }}" {{ old('supplier_id') == $supplier->id ? 'selected' : '' }} @if ($supplier->id == $goodReceiptNote->supplier_id) selected @endif>
                                {{ $supplier->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="payment_method_id">
                            ...
                            <option value="{{ $payment_method->id }}" {{ old('payment_method_id') == $payment_method->id ? 'selected' : '' }} @if ($payment_method->id == $goodReceiptNote->payment_method_id) selected @endif>
                                {{ $payment_method->name }}
                            </option>
                            @endforeach
                        </select>
